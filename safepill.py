...
def check_pin_lockout(phone: str) -> tuple[bool, int]:
    record = st.session_state.pin_attempts.get(phone)
    if not record:
        return False, 0
    ...
    if locked_until:
        ...
        st.session_state.pin_attempts[phone] = {"count": 0, "locked_until": None}
    return False, 0

def register_pin_failure(phone: str) -> None:
    record = st.session_state.pin_attempts.get(phone, {"count": 0, "locked_until": None})
    record["count"] += 1
    if record["count"] >= MAX_PIN_ATTEMPTS:
        record["locked_until"] = datetime.now() + timedelta(minutes=LOCKOUT_MINUTES)
    st.session_state.pin_attempts[phone] = record

def reset_pin_failures(phone: str) -> None:
    st.session_state.pin_attempts.pop(phone, None)
...
# =====================================================================================
    ...
    with c2:
        st.markdown("<h2 style='text-align:center;color:#006a62;'>🔐 Đăng nhập / Đăng ký</h2>", unsafe_allow_html=True)
            ...
            if method == "Mã PIN 4 số":
                ...
                if st.button("Đăng nhập", type="primary", use_container_width=True):
                    ...
                    if not l_phone_clean or not l_pin_clean:
                        ...
                        is_locked, seconds_left = check_pin_lockout(l_phone_clean)
                        if is_locked:
                            st.error(f"🚫 Tài khoản tạm thời bị khóa do nhập sai nhiều lần. Vui lòng thử lại sau {seconds_left // 60} phút {seconds_left % 60} giây.")
                        else:
                                ...
                                try:
                                    ...
                                    if res.data:
                                        ...
                                        if verify_pin(l_pin_clean, user_row.get("pin")):
                                            reset_pin_failures(l_phone_clean)
                                            load_profile_into_session(user_row)
                                            st.success("✅ Đăng nhập thành công!")
                                            st.rerun()
                                        else:
                                            register_pin_failure(l_phone_clean)
                ...
                st.warning("⚠️ Lưu ý: FaceID hiện tại đang ở chế độ Demo thử nghiệm.")
                    ...
                    with st.spinner("🔍 Đang đối chiếu dữ liệu sinh trắc học..."):
                        try:
                            ...
                            if not login_hash:
                                ...
                                try:
                                    res = supabase.table(TABLE).select("phone, full_name, face_hash").execute()
...
# =====================================================================================
    ...
    with tab_home:
        ...
        if not med_data_valid and not st.session_state.custom_reminders and not family_reminder_payload:
            ...
            reminder_json = json.dumps(reminder_payload, ensure_ascii=False).replace("</script>", "<\\/script>")
...
"""
        def _image_hash_key(image_bytes: bytes) -> str:
            """Tạo khóa cache dựa trên nội dung ảnh, để không gọi lại Gemini cho ảnh giống hệt."""
            return hashlib.sha256(image_bytes).hexdigest()

        @st.cache_data(show_spinner=False, ttl=3600)
        def _cached_gemini_ocr(image_hash: str, image_bytes: bytes) -> str:
            """
            Cache kết quả OCR theo nội dung ảnh trong 1 giờ, giảm số lần gọi Gemini API
            không cần thiết khi người dùng vô tình quét lại cùng một ảnh — góp phần giảm
            tải tính toán và tác động môi trường của việc gọi AI.
            """
        ...
        def analyze_prescription_image(pil_img, clinic_override: str, doctor_override: str,
                                        pharmacy_override: str, image_bytes: bytes = None):
            """
            MỚI — Hàm dùng CHUNG để gọi AI Gemini bóc tách 1 ảnh đơn thuốc/hồ sơ bệnh án, dù ảnh đến
            từ camera (st.camera_input) hay tải lên từ máy (st.file_uploader). Nếu image_bytes được
            truyền vào, dùng cache theo nội dung ảnh để giảm gọi API thừa.
            """
            if image_bytes is not None:
                img_hash = _image_hash_key(image_bytes)
                raw_text = _cached_gemini_ocr(img_hash, image_bytes)
            else:
                response = ai_gemini.models.generate_content(
                    model="gemini-flash-latest",
                    contents=[pil_img, PRESCRIPTION_VISION_PROMPT],
            ...
            parsed_meds = extract_json_array(raw_text)
        ...
        if has_prescription.startswith("📷"):
            ...
            if img_file:
                with st.spinner("🤖 Đang phân tích hình ảnh bằng AI..."):
                    try:
                        img_bytes_raw = img_file.getvalue()
                        pil_img = Image.open(io.BytesIO(img_bytes_raw))
                        parsed_meds = analyze_prescription_image(
                            pil_img, rx_clinic, rx_doctor, rx_pharmacy, image_bytes=img_bytes_raw
                        )
                        if not parsed_meds:
                    ...
                    with st.spinner(f"🤖 Đang phân tích {len(uploaded_files)} ảnh bằng AI..."):
                        for uf in uploaded_files:
                            try:
                                ...
                                pil_img = Image.open(io.BytesIO(uf_bytes))
                                parsed_meds = analyze_prescription_image(
                                    pil_img, rx_clinic, rx_doctor, rx_pharmacy, image_bytes=uf_bytes
                                )
